...
export default function HubVenuesPage() {
  ...
  const filterOptions = useMemo(() => {
    ...
    const statuses = Array.from(new Set(venues.map((v) => v.status))).sort();
    const cities = Array.from(new Set(venues.map((v) => v.city).filter(Boolean))).sort();
    return { venueTypes, statuses, cities };
  }, [venues]);
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        key: "status",
        header: "Status",
        render: (r: VenueItem) => r.status,
        sortable: true,
        sortValue: (r: VenueItem) => r.status,
      },
      {
        key: "city",
        header: "City",
  ...
  if (loading) {
    return (
      <div>
        <h1 className="font-serif text-3xl font-bold text-foreground">Venues</h1>
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-2 text-muted-foreground">
              Manage venue profiles, tables, and event tickets.
            </p>
          </div>
          <div className="flex flex-1 flex-wrap items-center gap-2 sm:flex-initial sm:justify-end">
            ...
            <ViewToggle />
            <Button size="sm">
              <Link href="/hub/venues/new">Add venue</Link>
            </Button>
          </div>
        </div>
