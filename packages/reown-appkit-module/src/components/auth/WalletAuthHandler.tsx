...
export function WalletAuthHandler({
  ...
  // Get chain ID for network name - use wagmi first, then AppKit network as fallback
  const wagmiChainId = useChainId();
  const appKitNetwork = useAppKitNetwork();
  // Ensure chainId is a number (wagmi returns number, AppKit might return string)
  const chainId = wagmiChainId || (appKitNetwork?.chainId ? Number(appKitNetwork.chainId) : undefined);
  ...
  useEffect(() => {
    const handleTimeoutWarning = (event: CustomEvent) => {
