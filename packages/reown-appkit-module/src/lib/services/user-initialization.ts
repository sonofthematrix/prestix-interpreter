 ...
 * - UserQtechAccount (Gaming account for QTech integration)
...
export async function initializeNewUser(
  ...
  // Use system user context for user initialization (admin operation)
  ...
  try {
    ...
    // Use minimal query first to avoid schema validation errors
    ...
    // Check for accounts separately to avoid schema validation errors
    // Only query relations if they exist in the schema
    let accounts: any[] = [];
    try {
      // Try to query accounts relation if it exists
      ...
      // If accounts relation doesn't exist, try to query Account model directly
      try {
        ...
        // If Account model doesn't exist, assume no accounts
        console.warn('⚠️ [User Init] Account model/relation not found in schema, skipping account check');
        accounts = [];
      }
    }
    ...
    if (isNewUser || accounts.length === 0) {
    ...
    // 4. Create UserQtechAccount (Gaming Account) if it doesn't exist
    // Note: userQtechAccount is an array, but typically one per user
    if (isNewUser && (!existingUser.userQtechAccount || existingUser.userQtechAccount.length === 0)) {
      try {
        await db.userQtechAccount.create({
          data: {
            userId: options.userId,
            status: 'ACTIVE',
          },
        });
        console.log('✅ UserQtechAccount (Gaming Account) created');
      } catch (error: any) {
        errors.push(`UserQtechAccount creation failed: ${error.message}`);
        console.error('⚠️ UserQtechAccount creation error:', error);
      }
    }

    // 5. Create Portfolio (empty) if it doesn't exist
    // Note: portfolio is an array, but typically one per user
    if (isNewUser && (!existingUser.portfolio || existingUser.portfolio.length === 0)) {
      try {
        await db.portfolio.create({
          data: {
            userId: options.userId,
            // Empty portfolio - no realEstateAssets connected yet
          },
        });
...
export async function initializeNewUserTransactional(
  ...
  try {
    return await (await db).$transaction(async (tx: any) => {
...
export async function getOnboardingStatus(userId: string): Promise<{
  ...
  const notifications = await (await db).notification.findMany({
    where: {
      recipientId: userId,
    },
  });
  
  // Filter for onboarding tasks using JavaScript (since JSON path queries aren't fully supported)
  const onboardingNotifications = notifications.filter(n => {
    const data = n.data as any;
    return data && data.onboardingTask === true;
  });
  
  // Check which tasks have been completed (user visited the page)
