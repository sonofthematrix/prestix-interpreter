...
// Fallback USD price for native token when price API fails (e.g. Sepolia testnet)
function getNetworkTokenPriceFallback(
  chainId: string | number | undefined
): { price: number; symbol: string } | null {
  const id = String(chainId ?? '')
  if (id === '11155111') return { price: 3000, symbol: 'ETH' } // Sepolia
  if (id === '1') return { price: 3000, symbol: 'ETH' } // Mainnet rough fallback
  return null
}
...
const controller = {
  ...
  async getNetworkTokenPrice() {
    // Use getActiveNetworkTokenAddress() so we don't throw when user address isn't ready yet (e.g. Send view opening before social wallet resolves)
    const networkAddress = getActiveNetworkTokenAddress()
    const chainId = ChainController.state.activeCaipNetwork?.id

    const response = await BlockchainApiController.fetchTokenPrice({
      addresses: [networkAddress]
    }).catch((): BlockchainApiTokenPriceResponse => ({ fungibles: [] }))
    const token = response?.fungibles?.[0]
    let price = token?.price?.toString() ?? '0'
    let symbol = token?.symbol ?? ''

    // Fallback for testnets / when API returns no price (e.g. Sepolia) – avoid showing "Failed to fetch network token price"
    if (price === '0' || !token) {
      const fallback = getNetworkTokenPriceFallback(chainId)
      if (fallback) {
    ...
    state.networkTokenSymbol = symbol
