...
export const ApiController = {
  ...
  async fetchNetworkImages() {
    ...
    const ids = requestedCaipNetworks
      ...
      .filter(Boolean)
      .filter(imageId => !AssetController.state.networkImages[imageId as string ?? ''])
  ...
  async prefetchWalletRanks() {
    ...
    if (ChainController.state.activeChain === 'eip155' as ChainNamespace) {
      params.rdns = connectors.map(c => c.info?.rdns).filter(Boolean).join(',')
    }
  },
