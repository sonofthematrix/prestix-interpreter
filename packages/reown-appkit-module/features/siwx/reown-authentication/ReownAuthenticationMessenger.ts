...
import type { SIWXMessage } from '../../../src/utils/SIWXUtil.js'
...
export class ReownAuthenticationMessenger {
  ...
  private getNetworkName(chainId: CaipNetworkId): string | undefined {
    ...
    // Find the network matching the chainId
    const network = requestedNetworks.find(n => n.id === chainId)
    
    if (network) {
      return network.name
    }
    
    // Fallback: return name of first requested network
    return requestedNetworks[0]?.name
  }
